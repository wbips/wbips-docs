...
- Object
  ...
  - `addresses`: Array <span class="badge">Optional</span>
    - `address`: string
    - `derivationPath`: string
    - `publicKey`: string

<Callout type="info" emoji="ℹ️">
  The `AddressObjArray` here can be returned for on-chain wallets but can also
  be used by WebLN wallets which also expose the on-chain wallet functions. This
  typically is used to show that a wallet is connected to the page.
</Callout>
